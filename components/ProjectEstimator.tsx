...
const basePrices: Record<'web' | 'mobile' | 'both', number> = {
  web: 20000,
  mobile: 35000,
  both: 50000,
}
...
export default function ProjectEstimator() {
  ...
  const calculateTotal = () => {
    const parsedBase = Number(basePriceInput)
            ...
            >
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    <motion.button
                      key={type.value}
                      onClick={() => {
                        setProjectType(type.value as typeof projectType)
                        setBasePriceInput(basePrices[type.value as typeof projectType].toString())
                      }}
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-400">
                        Base:{' '}
                        ₹{basePrices[type.value as typeof projectType].toLocaleString('en-IN')}
                      </div>
                    </motion.button>
                  ))}
                </div>
              ...
              <div className="mb-8">
                <label className="block text-sm text-gray-400 mb-3">
                  Custom Base Price (INR)
                </label>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                  <div className="flex-1 flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 focus-within:border-neon-purple transition-all">
                  ...
                  <button
                    type="button"
                    onClick={() => setBasePriceInput(basePrices[projectType].toString())}
                    className="px-5 py-3 rounded-xl border border-white/10 text-sm text-gray-300 hover:bg-white/5 transition-all"
                  >
                    Use default
                  </button>
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  Start from the recommended base for the selected project type, then adjust to match
                  your specific scope or budget.
                </p>
              </div>
